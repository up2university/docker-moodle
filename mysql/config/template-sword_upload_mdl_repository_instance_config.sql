...
WHERE instanceid=(
  SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${SWORD_UPLOAD_REPO_NAME}') AND
  name='sword_url';
...
WHERE EXISTS (SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${SWORD_UPLOAD_REPO_NAME}' LIMIT 1) LIMIT 1;
COMMIT;
