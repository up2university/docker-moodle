...
('${PMKSEARCH_REPO_NAME}', (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${PMKSEARCH_REPOSITORY_TYPE}'),0,1,NULL,NULL,1516380035,1516380035,0)
WHERE EXISTS (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${PMKSEARCH_REPOSITORY_TYPE}');*/
/*INSERT INTO `mdl_repository` (type, visible, sortorder) SELECT '${PMKSEARCH_REPOSITORY_TYPE}', 1, (SELECT max(sortorder) + 1 FROM `mdl_repository`) FROM `mdl_repository`
WHERE NOT EXISTS (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${PMKSEARCH_REPOSITORY_TYPE}');*/
START TRANSACTION;
INSERT INTO `mdl_repository` (type, visible, sortorder)
SELECT '${PMKSEARCH_REPOSITORY_TYPE}', 1, (SELECT max(sortorder) + 1 FROM `mdl_repository` LIMIT 1)
FROM `mdl_repository`
WHERE NOT EXISTS (SELECT id FROM `mdl_repository`
  WHERE mdl_repository.type = '${PMKSEARCH_REPOSITORY_TYPE}' LIMIT 1) LIMIT 1;
COMMIT;
