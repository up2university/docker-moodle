...
INSERT INTO `mdl_oauth2_user_field_mapping` (timemodified, timecreated, usermodified, issuerid, externalfield, internalfield) VALUES (1516290823,1516290823,2,(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.name='${CERNBOX_NAME}' LIMIT 1),'ocs-data-email','email'),(1516290838,1516290838,2,(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.name='${CERNBOX_NAME}' LIMIT 1),'ocs-data-id','username');
