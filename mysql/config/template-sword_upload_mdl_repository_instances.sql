...
('${SWORD_UPLOAD_REPO_NAME}', (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${SWORD_UPLOAD_REPOSITORY_TYPE}'),0,1,NULL,NULL,1516380035,1516380035,0)
WHERE EXISTS (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${SWORD_UPLOAD_REPOSITORY_TYPE}');*/
/*INSERT INTO `mdl_repository` (type, visible, sortorder) SELECT '${SWORD_UPLOAD_REPOSITORY_TYPE}', 1, (SELECT max(sortorder) + 1 FROM `mdl_repository`) FROM `mdl_repository`
WHERE NOT EXISTS (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${SWORD_UPLOAD_REPOSITORY_TYPE}');*/
START TRANSACTION;
...
START TRANSACTION;
DELETE FROM `mdl_repository_instances` WHERE name='${SWORD_UPLOAD_REPO_NAME}' AND
typeid=(SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${SWORD_UPLOAD_REPOSITORY_TYPE}' LIMIT 1);
INSERT INTO `mdl_repository_instances` (name, typeid, userid, contextid, username, password, timecreated, timemodified,readonly)
SELECT '${SWORD_UPLOAD_REPO_NAME}', (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${SWORD_UPLOAD_REPOSITORY_TYPE}' LIMIT 1),0,1,NULL,NULL,1516380035,1516380035,0
FROM `mdl_repository`
WHERE EXISTS (SELECT id FROM `mdl_repository` WHERE mdl_repository.type = '${SWORD_UPLOAD_REPOSITORY_TYPE}' LIMIT 1 ) LIMIT 1;
COMMIT;
