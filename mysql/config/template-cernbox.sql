...
BEGIN
  IF EXISTS (SELECT * FROM `mdl_oauth2_issuer` WHERE baseurl='${CERNBOX_URL}') THEN
  ...
  IF EXISTS (SELECT issuerid from `mdl_oauth2_endpoint` 
  	where issuerid=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}'))
  	THEN
  	
  	UPDATE `mdl_oauth2_endpoint` set url='${CERNBOX_TOKEN_ENDPOINT}' where name='token_endpoint' 
  		AND issuerid=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}');

  	UPDATE `mdl_oauth2_endpoint` set url='${CERNBOX_AUTHORIZE_ENDPOINT}' where name='authorization_endpoint' 
  		AND issuerid=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}');

  	UPDATE `mdl_oauth2_endpoint` set url='${CERNBOX_WEBDAV_ENDPOINT}' where name='webdav_endpoint' 
  		AND issuerid=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}');
  ...
  IF NOT EXISTS (SELECT externalfield from `mdl_oauth2_user_field_mapping` 
  	where externalfield='ocs-data-email' AND issuerid=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}'))
  	THEN 
  ...
  IF NOT EXISTS (SELECT externalfield from `mdl_oauth2_user_field_mapping` 
  	where externalfield='ocs-data-id' AND issuerid=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}'))
  	THEN 
  ...
  IF EXISTS (SELECT instanceid FROM `mdl_repository_instance_config`
               ...
               value=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}')) THEN
      UPDATE `mdl_repository_instances`
      SET name='${CERNBOX_REPO_NAME}'
      WHERE id=(SELECT instanceid FROM `mdl_repository_instance_config`
                 WHERE name='issuerid' AND
                 value=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}')) 
      AND
  ...
  IF NOT EXISTS (SELECT id from `mdl_repository_instance_config` WHERE name='issuerid'
                 AND value=(SELECT id FROM `mdl_oauth2_issuer` WHERE mdl_oauth2_issuer.baseurl='${CERNBOX_URL}')) THEN
    INSERT INTO `mdl_repository_instance_config` (instanceid, name, value)
      ...
      WHERE EXISTS
        (SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${CERNBOX_REPO_NAME}' LIMIT 1) LIMIT 1;
  END IF;
END $$
delimiter ;
