...
WHERE instanceid=(
  SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1) AND
  name='pmksearchrepositoryurl';
INSERT INTO `mdl_repository_instance_config` (instanceid, name, value)
SELECT
(SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1),
'pmksearchrepositoryurl',
'${PMKSEARCH_REPOSITORY_URL}'
FROM `mdl_repository_instances`
WHERE EXISTS (SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1) LIMIT 1;
COMMIT;
...
WHERE instanceid=(
  SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1) AND
  name='pmksearchrepository_secret';
INSERT INTO `mdl_repository_instance_config` (instanceid, name, value)
SELECT
(SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1),
'pmksearchrepositorysecret',
...
WHERE instanceid=(
  SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1) AND
  name='pmksearch_ticket_field';
INSERT INTO `mdl_repository_instance_config` (instanceid, name, value)
SELECT
(SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1),
'pmksearch_ticket_field',
'${PMKSEARCH_TICKET_FIELD}'
FROM `mdl_repository_instances`
WHERE EXISTS (SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${PMKSEARCH_REPO_NAME}' LIMIT 1) LIMIT 1;
COMMIT;
