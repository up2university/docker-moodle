...
WHERE instanceid=(
  SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${OWNCLOUD_REPO_NAME}' LIMIT 1) AND
  name='issuerid';
INSERT INTO `mdl_repository_instance_config` (instanceid, name, value)
SELECT
(SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${OWNCLOUD_REPO_NAME}' LIMIT 1),
'issuerid',
(SELECT id from `mdl_repository` WHERE mdl_repository.type='${OWNCLOUD_REPOSITORY_TYPE}' LIMIT 1)
FROM `mdl_repository_instances`
WHERE EXISTS (SELECT id FROM `mdl_repository_instances` WHERE mdl_repository_instances.name='${OWNCLOUD_REPO_NAME}' LIMIT 1) LIMIT 1;
COMMIT;
